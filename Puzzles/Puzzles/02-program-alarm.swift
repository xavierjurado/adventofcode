...
class ProgramAlarm {
    ...
    func solve(program: [Int]) -> Int {
        ...
        guard let memory = try? execute(program: program) else {
            fatalError()
        }
    ...
    private func setup(program: [Int], noun: Int, verb: Int) -> [Int] {
        var program = program
        program[1] = noun
        program[2] = verb
        return program
    ...
    func solvePartTwo(program: [Int]) -> Int {
        ...
        for noun in 0...99 {
            for verb in 0...99 {
                ...
                guard let memory = try? execute(program: program) else { continue }
